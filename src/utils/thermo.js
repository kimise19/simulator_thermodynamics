...
export function calculateSimulation(process, inputs) {
  ...
  let W = 0, Q = 0, deltaU = 0, deltaH = 0, W_vol = 0, W_pres = 0;
  let mathSteps = [];
  let explanation = "";
  ...
  switch (process) {
    case 'isotermico': {
      ...
      explanation = `En este proceso isotérmico (temperatura constante a ${convertTemp.fromSI(Ti, inputs.unitT).toFixed(1)}°${inputs.unitT}), ` +
        ...
        `Todo el calor transferido al sistema (${Q.toFixed(1)} J) se convierte en trabajo mecánico (${W_vol.toFixed(1)} J). ` +
        `El trabajo de volumen (frontera) y el de presión son iguales (${W_vol.toFixed(1)} J), y no hay variación de energía interna ni de entalpía (ΔU = ΔH = 0).`;
      break;
    }
    ...
    case 'isobarico': {
      ...
      explanation = `En un proceso isobárico (presión constante a ${convertPressure.fromSI(Pi, inputs.unitP).toFixed(1)} ${inputs.unitP}), ` +
        ...
        `El calor absorbido/liberado es de ${Q.toFixed(1)} J, el cual equivale al cambio total de entalpía (ΔH = ${deltaH.toFixed(1)} J), ` +
        `empleándose una parte en aumentar la energía interna (ΔU = ${deltaU.toFixed(1)} J) y la otra en realizar trabajo.`;
      break;
    }
    ...
    case 'isocorico': {
      ...
      explanation = `En este proceso isocórico (volumen constante a ${convertVolume.fromSI(Vi, inputs.unitV).toFixed(3)} ${inputs.unitV}), ` +
        ...
        `Todo el calor transferido (${Q.toFixed(1)} J) se traduce directamente en un cambio proporcional en la energía interna (ΔU = ${deltaU.toFixed(1)} J) ` +
    ...
    case 'adiabatico': {
      ...
      mathSteps = [
        ...
        },
        {
          ...
          formula: `W_{vol} = \\frac{P_i V_i - P_f V_f}{\\gamma - 1} = \\frac{${Pi.toFixed(0)} \\cdot ${Vi.toFixed(4)} - ${Pf.toFixed(0)} \\cdot ${Vf.toFixed(4)}}{${gamma} - 1} = ${W_vol.toFixed(2)} \\text{ J}`
        },
        {
          ...
          formula: `W_{pres} = -\\int V dP = \\gamma W_{vol} = ${gamma} \\cdot ${W_vol.toFixed(2)} = ${W_pres.toFixed(2)} \\text{ J}`
        },
        {
      ...
      explanation = `En un proceso adiabático, el sistema no intercambia calor (Q = 0 J). ` +
        ...
        `y un enfriamiento de ${convertTemp.fromSI(Ti, inputs.unitT).toFixed(1)}°${inputs.unitT} a ${convertTemp.fromSI(Tf, inputs.unitT).toFixed(1)}°${inputs.unitT}.`;
      break;
    }
  }
